...
The container listens on `4263` and Compose publishes it as `4263:4263`. Set `APP_PORT` in `.env` to publish it on a different host port — the container's own port never changes, so a reverse proxy reaching `mc-gamertime` over the Docker network keeps targeting `4263`.

<Aside type="caution">
`APP_PORT` is read by docker-compose itself, not by the app. It is deliberately absent from the `mc-gamertime` service's `environment:` block — putting it there does nothing.
</Aside>
